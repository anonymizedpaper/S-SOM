...
def compute_region_noise(weighted_feature_sum, area):
    """Area-weighted spherical variance of a region's face normals, in [0, 1].

    weighted_feature_sum = Σ aᵢ·nᵢ  (area-weighted resultant of the unit face normals)
    area                = Σ aᵢ
    ...
    if area < 1e-12:
        return 0.0
    return float(1.0 - np.linalg.norm(weighted_feature_sum) / area)

def compute_noise_term(mean1, mean2, metric="euclidean",
                       theta0_deg=15.0, sharpness_deg=0.3,
                       dist0=0.15, sharpness_dist=20.0):
    """Feature-similarity gate N ∈ [0, 1] for merging two regions.

    Works for features of any dimension (1-D scalars, 2-D, 3-D vectors). `mean1`
    and `mean2` are the two regions' area-weighted MEAN feature vectors.

    metric="angular" — for unit-direction features (e.g. face Normals). Uses the
        angle θ between the two mean directions:
            N = 1 / (1 + exp(sharpness_deg · (θ_deg − theta0_deg)))
        Aligned (θ small) → 1; differently oriented (θ large) → 0. Magnitude is
        ignored, so it is meaningless for scalar features — use "euclidean" there.
        Returns 0.5 if either mean has no direction (degenerate).
        Defaults: 0°→0.99, 15°→0.5, 30°→0.011, 42°→0.0003.

    metric="euclidean" — for general scalar/vector features (e.g. SDF, curvature).
        Uses the Euclidean distance d = ‖mean1 − mean2‖ between the mean vectors:
            N = 1 / (1 + exp(sharpness_dist · (d − dist0)))
        Similar feature values (small d) → 1; dissimilar (large d) → 0.
        Defaults assume features are normalized to roughly [0, 1].
    """
    mean1 = np.atleast_1d(np.asarray(mean1, dtype=float))
    mean2 = np.atleast_1d(np.asarray(mean2, dtype=float))
    if metric == "angular":
        n1 = float(np.linalg.norm(mean1))
        n2 = float(np.linalg.norm(mean2))
        if n1 < 1e-12 or n2 < 1e-12:
            return 0.5
        cos_theta = float(np.dot(mean1, mean2)) / (n1 * n2)
        theta_deg = float(np.degrees(np.arccos(np.clip(cos_theta, -1.0, 1.0))))
        return float(1.0 / (1.0 + np.exp(sharpness_deg * (theta_deg - theta0_deg))))
    # euclidean (default): distance between mean feature vectors
    d = float(np.linalg.norm(mean1 - mean2))
    return float(1.0 / (1.0 + np.exp(sharpness_dist * (d - dist0))))
...
def merge_region_based_on_power(mesh, region_labels, face_adjacency,
                                alpha=3/4, beta=1/4, balance_k= 0.5,
                                power_thr=0.15, max_merges=1000,
                                target_n_regs=None, verbose=True, feature_name = "Normals", feature_metric=None):
    ...
    N - Feature gate : similarity of the two regions' area-weighted mean feature
        vectors, in [0,1]. Works for 1-D, 2-D and 3-D features. metric="angular"
        (unit-direction features like Normals) uses the angle between the mean
        directions; metric="euclidean" (scalar/vector features like SDF, curvature)
        uses the distance between the mean vectors. Similar → N→1; dissimilar → N→0.
        The metric is auto-selected from feature_name unless feature_metric is set.
        See compute_noise_term.
    ...
    if verbose:
        print(f"After merge_region_based_on_power: {len(region_labels)} regions remain.")

    merged_labels = region_labels.copy()
    # PyVista squeezes single-component cell_data to 1-D, so force (N, D) here:
    # this makes the feature handling work for 1-D (e.g. SDF), 2-D (SDF+curvature)
    # and 3-D (Normals) features alike, and avoids broadcasting bugs below.
    feature_data = np.asarray(mesh.cell_data[feature_name])
    if feature_data.ndim == 1:
        feature_data = feature_data.reshape(-1, 1)
    face_areas = compute_face_areas(mesh)

    # Feature-space metric: angle for unit-direction features (Normals), Euclidean
    # distance for general scalar/vector features. Override with feature_metric.
    if feature_metric is None:
        feature_metric = "angular" if feature_name == "Normals" else "euclidean"
    ...
    weighted_feature_sum = {
        rid: (face_areas[faces][:, None] * feature_data[faces]).sum(axis=0)
        for rid, faces in region_to_faces.items()
    }
    # Area-weighted mean feature vector per region; drives the feature gate N.
    region_mean = {rid: weighted_feature_sum[rid] / max(region_areas[rid], 1e-12)
    ...
    def power_of(r1, r2):
        ...
        n = compute_noise_term(region_mean[r1], region_mean[r2], metric=feature_metric)
    ...
    for merge_num in range(max_merges):
        ...
        feat_diff = float(np.linalg.norm(region_mean[r1] - region_mean[r2]))
        if verbose:
            print(f"Merge {merge_num + 1}: {r1}+{r2}  "
                  f"areas=({area_r1:.2f},{area_r2:.2f})  ratio={ratio:.2f}  "
                  f"featDist={feat_diff:.3f}  "
                  f"P={p_fresh:.4f} (aS={alpha*s:.3f} N={n_val:.3f} R={r_val:.3f})  "
        ...
        # 3. Update the pooled weighted feature sum and the region mean feature
        #    (region_areas[target_id] was already updated in step 2 above).
        weighted_feature_sum[target_id] = (
            weighted_feature_sum[target_id] + weighted_feature_sum.pop(merged_id))
        region_mean[target_id] = weighted_feature_sum[target_id] / max(region_areas[target_id], 1e-12)
