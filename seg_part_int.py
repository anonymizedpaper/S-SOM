...
def plot(plotter, mesh, scalar_name, class_count):
    ...
    if(class_count <20):
        return plotter.add_mesh(mesh, scalars=scalar_name,  cmap = colors, show_scalar_bar=True, show_edges=True, edge_opacity=0.2, scalar_bar_args={"fmt": "%.0f", "n_labels": class_count})
    else:
        return plotter.add_mesh(mesh, scalars=scalar_name,  cmap = colors, show_scalar_bar=True, show_edges=True, edge_opacity=0.2, scalar_bar_args={"fmt": "%.0f", "n_labels": 20})
def plot_with_title(plotter, mesh, scalar_name, class_count, title):
    assert(class_count > 0)
    colors = get_color_map(class_count)
    if(class_count <20):
        return plotter.add_mesh(mesh, scalars=scalar_name,  cmap = colors, show_scalar_bar=True, show_edges=True, edge_opacity=0.2, scalar_bar_args={"fmt": "%.0f", "n_labels": class_count, "title": title})
    else:
        return plotter.add_mesh(mesh, scalars=scalar_name,  cmap = colors, show_scalar_bar=True, show_edges=True, edge_opacity=0.2, scalar_bar_args={"fmt": "%.0f", "n_labels": 20, "title": title})
...
def process_obj_file(obj_file, feature_name, sdf_coneangle, lr, radius, min_region_face_count, threshold_similarity_merge,  visualize=True):   
    ...
    if feature_name == "sdf": # Select feature set based on CLI argument        
        ...
        ms.compute_scalar_by_shape_diameter_function_per_vertex_gpu(coneangle = sdf_coneangle, onprimitive = 'On Faces', numberrays = 180)
        ...
        features = cur_normalized.reshape(-1, 1)
        norms = np.linalg.norm(features, axis=1)
        ...
        ms = pymeshlab.MeshSet()
        ms.load_new_mesh(obj_file)
        ms.compute_scalar_by_shape_diameter_function_per_vertex_gpu(coneangle = sdf_coneangle, onprimitive = 'On Faces', removeoutliers = True, numberrays = 180)
        sdf = ms.current_mesh().face_scalar_array()
        sdf = sdf/np.max(sdf)
    ...
    def merge_regions_feature_based(threshold):
        merged_region_label_temp = merged_small_region_labels.copy()
        merged_region_label_temp = merge_similar_neighbor_regions(obj_mesh, merged_region_label_temp, face_adjacency, threshold)
        merged_similar_region_labels, labels_count = remap_labels(merged_region_label_temp)  # Convert to face labels 0-based indices
        ...
        return merged_similar_region_labels, labels_count
    obj_mesh.cell_data["merged_similar_region_labels"], merged_similar_region_labels_count = merge_regions_feature_based(threshold_similarity_merge)
    ...
    if visualize:
        ...
        obj_mesh02 = obj_mesh.copy()
        plot(plotter,obj_mesh02, "merged_small_region_labels", merged_region_labels_count)
        
        plotter.subplot(1, 0)
        plotter.add_text("Final Segmentation", font_size=12)
        obj_mesh10 = obj_mesh.copy()
        final_seg_actor = [plot_with_title(plotter, obj_mesh10, "merged_similar_region_labels", merged_similar_region_labels_count, "Segment ID")]
        
        if sdf is not None: # Add SDF visualization if declared
            plotter.subplot(1, 1)
        ...
        if cur_normalized is not None:# Add curvature visualization if declared
            plotter.subplot(1, 2)
        ...
        # Slider to interactively tune similarity threshold and update subplot (1, 0)
        def _update_similarity_threshold(value):
            # Update the mesh data
            obj_mesh.cell_data["merged_similar_region_labels"], merged_similar_region_labels_count = merge_regions_feature_based(value)
            print("Update region merged_similar_region_labels_count", merged_similar_region_labels_count)
            
            # Switch to subplot (1, 0) and update the visualization
            plotter.subplot(1, 0)
            
            # Remove the old actor
            if final_seg_actor[0] is not None:
                plotter.remove_actor(final_seg_actor[0])
            
            # Create a new mesh copy with updated data
            obj_mesh10_updated = obj_mesh.copy()            
            # Add the updated mesh and update the actor reference (modify list element, don't create new variable)
            final_seg_actor[0] = plot_with_title(plotter, obj_mesh10_updated, "merged_similar_region_labels", merged_similar_region_labels_count, "Segment ID")
            
            # Force render
            plotter.render()

        # Position slider in subplot(1, 0)
        plotter.subplot(1, 0)
        plotter.add_slider_widget(callback=_update_similarity_threshold, rng=[0.0, 0.9], value=threshold_similarity_merge, title="Similarity threshold for region merging")

        plotter.show()


def main(input, fea, sdf_coneangle, lr, radius, min_region_face_count, threshold_similarity_merge):
    ...
    if os.path.isdir(input):
        ...
        for obj_path in obj_files:
            process_obj_file(obj_path, fea, sdf_coneangle, lr, radius, min_region_face_count, threshold_similarity_merge, visualize=False)
    else:
        # Process single file with visualization
        process_obj_file(input, fea, sdf_coneangle, lr, radius, min_region_face_count, threshold_similarity_merge, visualize=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sdf_coneangle", type=float, default=160, help="Angle of the cone to compute sdf if fea use to sdf")
    ...
    parser.add_argument("--thr", type=float, default=0.05, help="Similarity threshold for region merging")
    args = parser.parse_args()
    main(args.input, args.fea, args.sdf_coneangle, args.lr, args.radius, args.min_region_face_count, args.thr)
