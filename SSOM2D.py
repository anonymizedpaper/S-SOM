...
class SphereSOM:
    ...
    def _initialize_nodes(self, n_dim):
        self.weights = np.ones((self.n_nodes, n_dim), dtype=np.float32)*100
...
def remap_labels(labels):
    ...
    label_map = {label: i for i, label in enumerate(unique_labels, 0)}
...
def merge_similar_neighbor_regions(mesh, region_labels, face_adjacency, threshold_similarity_merge):
    print("threshod in merging",threshold_similarity_merge)
    ...
    while still_having_similar_regions:
        ...
        for (r1, n1), (r2, n2) in itertools.combinations(avg_normals.items(), 2):  # Find similar adjacent pairs and merge them
            ...
            if  r1 in region_adjacency.get(r2, set()):
                similarity = np.linalg.norm(n1 - n2)  # Euclidean distance
                #print(r1, r2, similarity)
                if similarity < threshold_similarity_merge:
                    print(f"Region {r1} and Region {r2} have similar features (Euclidean distance = {similarity:.3f})")
                    merged_id = -1
                    more_common_id = -1
